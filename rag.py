...
def load_policy_pdf(pdf_path: str = POLICY_PDF):
    """
    ...
    for page_num, page in enumerate(doc):
        ...
        if not text:
            continue
        # Split page into ~300-char chunks with 50-char overlap
        step = 300
        overlap = 50
        start = 0
        while start < len(text):
            ...
            if len(chunk) > 30:  # skip tiny fragments
                chunks.append({
                    "id": f"page{page_num}_chunk{start}",
                    "text": chunk,
...
def retrieve_policy(query: str, n_results: int = 4) -> list[str]:
    """
